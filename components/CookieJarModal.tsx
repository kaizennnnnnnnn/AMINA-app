...
// ── cookie path generator (smooth irregular baked shape) ─────────
function cookiePath(r: number): string {
  ...
  for (let i = 0; i < N; i++) {
    ...
    const p3 = pts[(i + 2) % N];
    const t = 0.38;
    const cp1x = p1[0] + t * (p2[0] - p0[0]) / 3;
    const cp1y = p1[1] + t * (p2[1] - p0[1]) / 3;
    const cp2x = p2[0] - t * (p3[0] - p1[0]) / 3;
    const cp2y = p2[1] - t * (p3[1] - p1[1]) / 3;
...
export default function CookieJarModal({
  coupleId, userId, myName, partnerName, onClose,
}: {
  ...
  useEffect(() => {
    ...
    type Cookie = {
      ...
      g: Element; settleScore: number; sleeping: boolean;
    };

    let cookies: Cookie[] = [];
    let lastTs = 0, spawnT = 0, elapsed = 0;
    let st = 'fill', stT = 0;
    let topClosed = false, shakeX = 0, shakeV = 0, spawned = 0;
    let animId = 0;
    ...
    const innerHW = (y: number) => y <= C.neckY
      ? C.nHW + (C.bHW - C.nHW) * clamp((y - C.openY)/(C.neckY - C.openY), 0, 1)
      : C.bHW;
    ...
    function makeCookie(): Cookie {
      ...
      hl.setAttribute('cx', (-r * 0.22).toFixed(1));
      hl.setAttribute('cy', (-r * 0.26).toFixed(1));
      hl.setAttribute('rx', (r * 0.38).toFixed(1));
      hl.setAttribute('ry', (r * 0.20).toFixed(1));
      ...
      const crackEls: Element[] = [];
      for (let k = 0; k < numCracks; k++) {
        const crack = document.createElementNS(NS, 'path');
        const sx = rand(-r * 0.35, r * 0.35), sy = rand(-r * 0.35, r * 0.35);
        const len = rand(r * 0.22, r * 0.48);
        const ang = rand(0, Math.PI);
        const ex = sx + Math.cos(ang) * len, ey = sy + Math.sin(ang) * len;
        const mx = (sx + ex) / 2 + rand(-r * 0.07, r * 0.07);
        const my = (sy + ey) / 2 + rand(-r * 0.07, r * 0.07);
        crack.setAttribute('d', `M ${sx.toFixed(1)},${sy.toFixed(1)} Q ${mx.toFixed(1)},${my.toFixed(1)} ${ex.toFixed(1)},${ey.toFixed(1)}`);
        crack.setAttribute('fill', 'none');
        crack.setAttribute('stroke', 'rgba(95,42,8,0.32)');
        crack.setAttribute('stroke-width', (0.7 + Math.random() * 0.5).toFixed(1));
        crack.setAttribute('stroke-linecap', 'round');
        crackEls.push(crack);
      }

      // ── chips: irregular ellipses ─────────────────────────────
      const chipPts: { x: number; y: number; r: number }[] = [];
      for (let i = 0; i < 6; i++) {
        for (let a = 0; a < 22; a++) {
          const ang2 = rand(0, Math.PI * 2), dist = rand(r * 0.10, r * 0.50);
          const cx2 = Math.cos(ang2) * dist, cy2 = Math.sin(ang2) * dist;
          const cr = rand(r * 0.08, r * 0.14);
          if (!chipPts.some(c => (cx2-c.x)**2+(cy2-c.y)**2 < (cr+c.r+r*0.1)**2)) {
            chipPts.push({ x: cx2, y: cy2, r: cr }); break;
          }
        }
      }

      const chipEls = chipPts.map(ch => {
        const c = document.createElementNS(NS, 'ellipse');
        c.setAttribute('cx', ch.x.toFixed(1));
        c.setAttribute('cy', ch.y.toFixed(1));
        c.setAttribute('rx', ch.r.toFixed(2));
        c.setAttribute('ry', (ch.r * rand(0.55, 0.85)).toFixed(2));
        c.setAttribute('fill', Math.random() > 0.5 ? '#3a1a08' : '#201008');
        c.setAttribute('opacity', rand(0.82, 0.97).toFixed(2));
      ...
      for (let s = 0; s < 4; s++) {
        ...
        const ang = rand(0, Math.PI * 2), dist = rand(r * 0.1, r * 0.6);
        sp.setAttribute('cx', (Math.cos(ang) * dist).toFixed(1));
        sp.setAttribute('cy', (Math.sin(ang) * dist).toFixed(1));
        sp.setAttribute('r', (rand(0.6, 1.4)).toFixed(1));
        sp.setAttribute('fill', 'rgba(255,240,200,0.55)');
        g.appendChild(sp);
      }
      ...
      g.appendChild(edge);
      g.appendChild(hl);
      for (const c of crackEls) g.appendChild(c);
      for (const c of chipEls) g.appendChild(c);
      cookieLayer.appendChild(g);

      return {
        x: C.cx + rand(-54, 54), y: rand(-165, -80),
        vx: rand(-24, 24), vy: rand(32, 78),
        r, rot: rand(0, 360), vr: rand(-10, 10),
        g, settleScore: 0, sleeping: false,
      };
    }
    ...
    function jarCollide(c: Cookie) {
      ...
      if (c.y > C.floorY - c.r) {
        c.y = C.floorY - c.r;
        if (c.vy > 0) { shakeV += Math.min(c.vy * 0.0012, 0.18); c.vy *= -0.12; c.vx *= 0.94; }
      }
    ...
    function physics(dt: number) {
      for (const c of cookies) {
        c.vy += C.gravity * dt; c.vx *= 1-C.air; c.vy *= 1-C.air*0.45;
        if (c.y <= C.openY + c.r) { c.vr *= 0.94; c.rot += c.vr * dt; } else c.vr = 0;
      ...
      for (let p = 0; p < 4; p++) {
        for (let i = 0; i < cookies.length; i++) {
          for (let j = i+1; j < cookies.length; j++) {
            ...
            const dx = b.x-a.x, dy = b.y-a.y, md = a.r+b.r, sq = dx*dx+dy*dy;
            if (sq < md*md && sq > 0) {
              const d = Math.sqrt(sq), ov = (md-d)*0.5, nx = dx/d, ny = dy/d;
              a.x -= nx*ov; a.y -= ny*ov; b.x += nx*ov; b.y += ny*ov;
              const rv = (b.vx-a.vx)*nx + (b.vy-a.vy)*ny;
              if (rv < 0) { const imp = -1.14*rv*0.5; a.vx -= imp*nx; a.vy -= imp*ny; b.vx += imp*nx; b.vy += imp*ny; }
            }
          }
        }
        for (const c of cookies) {
          jarCollide(c);
          c.x = clamp(c.x, C.cx - innerHW(c.y) + c.r, C.cx + innerHW(c.y) - c.r);
          c.y = Math.min(c.y, C.floorY - c.r);
        }
      }
      for (const c of cookies) {
        const spd = Math.hypot(c.vx, c.vy);
        c.settleScore = spd < 18 && c.y > 220 ? Math.min(c.settleScore+dt*2.2,1) : Math.max(c.settleScore-dt*1.3,0);
        c.sleeping = c.settleScore > 0.95;
      }
    }

    function tick(ts: number) {
      if (!lastTs) lastTs = ts;
      const dt = Math.min((ts-lastTs)/1000, 0.022);
      lastTs = ts;

      if (st === 'fill') {
        spawnT += dt; elapsed += dt;
        if (spawned < C.maxC && spawnT >= C.spawnInt) { spawnT = 0; cookies.push(makeCookie()); spawned++; }
        const settled = cookies.filter(c => c.sleeping).length;
        if ((spawned >= C.maxC && cookies.every(c => c.y > 115) && settled > C.maxC*0.5 && elapsed > 3.8) || (elapsed > 7.4 && spawned >= C.maxC))
          { st = 'closing'; stT = 0; }
      }
      if (st === 'closing') {
        stT += dt;
        const e = eio(clamp(stT/0.48,0,1));
        lid.x = 940 + (lid.tx-940)*e; lid.y = 24 + (lid.ay-24)*e; lid.rot = -16 + 20*e;
        if (stT >= 0.48) { st = 'twist'; stT = 0; }
      }
      if (st === 'twist') {
        stT += dt;
        const t = clamp(stT/0.82,0,1), e = eio(t);
        lid.x = lid.tx;
        lid.y = lid.ay + (lid.ty-lid.ay)*e + Math.sin(t*Math.PI*5)*(1-t)*1.2;
        lid.rot = 4.8*(1-e) + Math.sin(t*Math.PI*3)*(1-t)*2.4;
        if (t >= 1) { topClosed = true; lid.y = lid.ty; lid.rot = 0; st = 'sealed'; setPhase('ready'); }
      }
      ...
      physics(dt);
      for (const c of cookies) {
        c.g.setAttribute('transform', `translate(${c.x.toFixed(1)} ${c.y.toFixed(1)}) rotate(${c.rot.toFixed(1)})`);
      }
    ...
    return () => {
      ...
      while (cookieLayer.firstChild) cookieLayer.removeChild(cookieLayer.firstChild);
    };
  }, []);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4" onClick={onClose}>
      <div className="relative w-full max-w-sm bg-zinc-950 rounded-3xl shadow-2xl overflow-hidden" onClick={e => e.stopPropagation()}>

        {/* header */}
        <div className="flex items-center justify-between px-5 pt-4 pb-0">
          <div className="text-center min-w-[56px]">
            <p className="text-xs text-zinc-500">{myName}</p>
            <p className="text-xl font-bold leading-none">{myCnt} 🍪</p>
          </div>
          <div className="flex items-center gap-2">
            <button
              type="button"
              onClick={() => setView(v => v === 'jar' ? 'songs' : 'jar')}
              className={`btn text-xs px-3 py-1 ${view === 'songs' ? 'btn-primary' : 'btn-dark'}`}
            >
              {view === 'songs' ? '← Jar' : '🎵 Songs'}
            </button>
          </div>
          <div className="text-center min-w-[56px]">
            <p className="text-xs text-zinc-500">{partnerName}</p>
            <p className="text-xl font-bold leading-none">{partnerCnt} 🍪</p>
          </div>
        </div>

        {/* close */}
        <button type="button" className="absolute top-3 right-4 text-zinc-500 hover:text-white text-2xl leading-none z-10" onClick={onClose}>×</button>

        {/* ── JAR VIEW ──────────────────────────────────────────── */}
        <div className={view === 'jar' ? '' : 'hidden'}>
          <div
            className={`select-none transition-transform ${phase === 'ready' ? 'cursor-pointer active:scale-[0.98]' : ''}`}
            onClick={pull}
          >
            <svg viewBox="0 0 800 700" className="w-full" aria-label="Cookie jar">
              <defs>
          ...
          <div className="px-5 pb-5">
            {phase === 'intro' ? (
              ...
              <p className="text-center text-sm text-zinc-500">No songs yet — tap <strong>🎵 Songs</strong> to add some.</p>
            ) : revealed ? (
              <div className="space-y-2">
                <a href={revealed.url} target="_blank" rel="noopener noreferrer" className="btn btn-primary w-full text-center block text-sm">
              ...
              <p className="text-center text-xs text-zinc-400">🍪 Tap the jar to pull a cookie!</p>
            )}
          </div>
        </div>
        ...
        {/* ── SONGS VIEW ────────────────────────────────────────── */}
        {view === 'songs' && (
          <div className="px-5 pb-5 pt-3 space-y-4 max-h-[75vh] overflow-y-auto">
            {/* add form */}
            <div className="space-y-2">
              <h3 className="text-sm font-semibold text-zinc-300">Add a song</h3>
              <input
            ...
            {items.length > 0 && (
              <div className="space-y-2">
                <h3 className="text-sm font-semibold text-zinc-300">Songs in jar ({items.length})</h3>
                {items.map(item => (
                  <div key={item.id} className="flex items-center gap-2 rounded-xl border border-zinc-800 bg-zinc-900 px-3 py-2">
                    ...
                    <button
                      type="button"
                      className="text-zinc-600 hover:text-rose-400 text-lg leading-none flex-shrink-0"
            ...
            {items.length === 0 && (
              <p className="text-center text-sm text-zinc-500 py-4">No songs yet. Add the first one!</p>
            )}
          </div>
        )}
      </div>
